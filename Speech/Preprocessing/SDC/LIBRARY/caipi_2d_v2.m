function [ img_grappa, mask, mask_acs ] = caipi_2d_v1( kspace_sampled, kspace_acs, Rtot, Rz, c_del, acs_size, kernel_size, lambda_tik, subs )
...
num_acsY = acs_size(1);                % acs size
num_acsZ = acs_size(2);                % acs size
 

% acs masks
mask_acs = zeros(N);
mask_acs(1+end/2-num_acsY/2:end/2+num_acsY/2 + 1, 1+end/2-num_acsZ/2:end/2+num_acsZ/2 + 1) = 1;
...
Apad = ceil( (acs_size+1) / Rtot) * Rtot;
...
% kz_begin = 1 + Rz * kernel_hsize(2);            % first kernel center point that fits acs region 
kz_begin = 1 + pad_size(2);

% kz_end = num_acsZ - Rz * kernel_hsize(2) + 1;   % last kernel center point that fits acs region 
kz_end = num_acsZ - pad_size(2) + 1;
kz_end = kz_end - max(acs_delz);

% ky_begin = 1 + Ry * kernel_hsize(1);            % first kernel center point that fits acs region 
ky_begin = 1 + pad_size(1);

% ky_end = num_acsY - Ry * kernel_hsize(1) + 1;   % last kernel center point that fits acs region 
ky_end = num_acsY - pad_size(1) + 1;


% k-space limits for recon:
Kz_begin = 1 + pad_size(2);             % pad integer amount of caipi pattern size 
Kz_end = N_pad(2) - pad_size(2);        

Ky_begin = 1 + pad_size(1);             % pad integer amount of caipi pattern size 
Ky_end = N_pad(1) - pad_size(1);        




% cropped acs data
kspace_acs_crop = kspace_acs(1+end/2-num_acsY/2:end/2+num_acsY/2 + 1, 1+end/2-num_acsZ/2:end/2+num_acsZ/2 + 1, :);
...
for Kz = 1 : num_acsZ + 1
    for Ky = 1 : num_acsY + 1
        ...
        % current sliding window on acs data: 
        kspace_acs_now = kspace_acs_crop(Ky:end, Kz:end, :);        
        
        for kz = kz_begin : Rtot : kz_end
            
            if (kz_begin > size(kspace_acs_now,2)) || (kz_end > size(kspace_acs_now,2))
                continue
            end
            
            for ky = ky_begin : Rtot : ky_end

                if (ky_begin > size(kspace_acs_now,1)) || (ky_end > size(kspace_acs_now,1))
                    continue
                end
               ...
               for y = ky-kernel_hsize(1)*Ry : Ry : ky+kernel_hsize(1)*Ry
                    ...
                    if (acs_delz(y) + kz + kernel_hsize(2)*Rz) > size(kspace_acs_now,2)
                        continue
                    end
        ...
        for kz = kz_begin : Rtot : kz_end
            ...
            for ky = ky_begin : Rtot : ky_end
                ...
                for y = ky-kernel_hsize(1)*Ry : Ry : ky+kernel_hsize(1)*Ry
                    ...
                    acs(y_idx,:,:) = kspace_acs_now(y, acs_delz(y) + kz-kernel_hsize(2)*Rz : Rz : acs_delz(y) + kz+kernel_hsize(2)*Rz, :);
                    y_idx = y_idx + 1;
                ...
                idx = 1;
                for rz = 1:Rz-1
                    Rhs(ind,:,idx) = kspace_acs_now(ky, acs_delz(ky) + kz-rz, :);
                    idx = idx + 1;
                end    

                for ry = 1:Ry-1
                    for rz = 0:Rz-1
                        Rhs(ind,:,idx) = kspace_acs_now(ky-ry, acs_delz(ky) + kz-rz, :);
                        idx = idx + 1;
                    end    
                end
