...
%         end 
        ...
        for e = 1:ne
            ...
            X = zpad(X,ss,ss,nc);   % zpad to the closest diadic 
    		Xe(:,:,:,e) = W*(X); % apply wavelet
	
        end
            
        Xe = softThresh2(Xe,wavWeight); % threshold ( joint sparsity)
            
        for e = 1:ne

            X = W'*(Xe(:,:,:,e)); % get back the image
            X = crop(X,sx,sy,nc); % return to the original size
...
% apply joint sparsity soft-thresholding 
absy = sqrt(sum(sum(abs(y).^2,3),4));

unity = y./(repmat(absy,[1,1,size(y,3),size(y,4)])+eps);

res = absy-t;
res = (res + abs(res))/2;
x = unity.*repmat(res,[1,1,size(y,3),size(y,4)]);
