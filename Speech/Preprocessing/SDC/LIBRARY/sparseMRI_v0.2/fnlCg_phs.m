function x = fnlCg_phs(x0,params)
%-----------------------------------------------------------------------
%
% res = fnlCg(x0,params)
%
...
FTXFMtx = params.FT * (params.M .* x);
FTXFMtdx = params.FT * (params.M .* dx);
...
gradObj = params.M .* (params.FT' * (params.data - params.FT * (params.M .* x)));

% temp = 1i .* conj(x) .* gradObj;
% [norm(imag(temp(:))), norm(real(temp(:)))]

gradObj = 2 * real(1i .* conj(x) .* gradObj);
...
DtDx = params.TV'*(params.TV*x);

grad = -2 * real(1i .* conj(x) .* DtDx);
