function x = fnlCg_JCS_GRAPPA(x0,params)
%-----------------------------------------------------------------------
%
% res = fnlCg(x0,params)
%
...
FTXFMtx = zeros([params.N, params.num_chan, params.num_cycle]);
FTXFMtdx = zeros([params.N, params.num_chan, params.num_cycle]);
...
if params.TVWeight
    ...
    if params.TV_flag == 3
        TV = sum(sum(abs(w).^2, 3), 4).^0.5;
    else
        if params.TV_flag == 2
            TV = sum(abs(w).^2, 3).^0.5;
        else
            if params.TV_flag == 1
                TV = sum(abs(w).^2, 4).^0.5;
            else
                TV = abs(w);
            end
        end
...
X = zeros([params.N, params.num_chan, params.num_cycle]);
...
Dx = params.TV * X + params.b;      % x,y,chan,cycle,grad


if params.TV_flag == 3
    % joint over coils and cycles
    Dx_rsos = repmat(sum(sum(abs(Dx).^2, 3), 4).^0.5, [1,1,params.num_chan,params.num_cycle,1]);
else
    if params.TV_flag == 2
        % joint over coils
        Dx_rsos = repmat(sum(abs(Dx).^2, 3).^0.5, [1,1,params.num_chan,1,1]);
    else
        if params.TV_flag == 1
            % joint over cycles
            Dx_rsos = repmat(sum(abs(Dx).^2, 4).^0.5, [1,1,1,params.num_cycle,1]);
        else
            Dx_rsos = abs(Dx); 
        end
    end
