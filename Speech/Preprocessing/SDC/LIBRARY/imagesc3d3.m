function [  ] = imagesc3d3( img, pos, fig_num, rot_deg, scale_fig, avg_size, title_fig, MIP, axis_square )
...
if ~MIP
    if axis_square
        figure(fig_num), subplot(131), imagesc( abs(imrotate(squeeze(mean(img(pos(1)-Lavg_size:pos(1)+Ravg_size,:,:),1)), rot_deg(1))), scale_fig ), axis square off, colormap gray
        figure(fig_num), subplot(132), imagesc( abs(imrotate(squeeze(mean(img(:,pos(2)-Lavg_size:pos(2)+Ravg_size,:),2)), rot_deg(2))), scale_fig ), axis square off, colormap gray
            title(title_fig, 'color', 'w', 'fontsize', 24)
        figure(fig_num), subplot(133), imagesc( abs(imrotate(mean(img(:,:,pos(3)-Lavg_size:pos(3)+Ravg_size),3), rot_deg(3))), scale_fig ), axis square off, colormap gray
    else
        figure(fig_num), subplot(131), imagesc( abs(imrotate(squeeze(mean(img(pos(1)-Lavg_size:pos(1)+Ravg_size,:,:),1)), rot_deg(1))), scale_fig ), axis image off, colormap gray
        figure(fig_num), subplot(132), imagesc( abs(imrotate(squeeze(mean(img(:,pos(2)-Lavg_size:pos(2)+Ravg_size,:),2)), rot_deg(2))), scale_fig ), axis image off, colormap gray
            title(title_fig, 'color', 'w', 'fontsize', 24)
        figure(fig_num), subplot(133), imagesc( abs(imrotate(mean(img(:,:,pos(3)-Lavg_size:pos(3)+Ravg_size),3), rot_deg(3))), scale_fig ), axis image off, colormap gray
    end
else
    if axis_square
