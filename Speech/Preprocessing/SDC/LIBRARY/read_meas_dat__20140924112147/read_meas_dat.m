...
% $Id: read_meas_dat.m,v 1.19 2012/02/26 18:54:08 jonp Exp $
%**************************************************************************%
  ...
  % this causes trouble in some matlabs -- comment out for now
...
%  scan_phascor2d           = matrixtype([]);
  ...
  patrefscan_dpg               = matrixtype([]);
%  patrefscan               = {matrixtype([])};
%  patrefscan               = {[]};
%  patrefscan               = cell(1);
%  scan_patrefscan          = matrixtype([]);
  FLAG__patrefscan         = 0;
  FLAG__patrefscan_dpg      = 0;
...
%      fseek(fp, fpos_rep, 'bof');
      ...
      for ichannel = 1:nchannel,
        ...
        if ( read_meas__extract_bit(mdh(idx).aulEvalInfoMask(1), EvalInfoMask.MDH_RTFEEDBACK) ),
          ...
          % for VD11 data, since some Skyra sequences use DORK, which involves
          % phase correction lines to be processed via RTFEEDBACK. These lines
        ...
        if ( ~FLAG__stored_channel_indices ),
          if ( ~ismember(pos(03), channel_indices) ),
            ...
            if ( DO__SQUEEZE_CHANNELS && ...
              ...
              if ( FLAG__patrefscan_dpg ),         patrefscan_dpg         = patrefscan_dpg(           :,:,channel_sort,:,:,:,:,:,:,:,:,:,:,:,:,:); end;
...
%           if ( visited ), warning(sprintf('visited line detected: scan number %d', scan_num)); end;
          ...
          elseif ( read_meas__extract_bit(mdh(idx).aulEvalInfoMask(1), EvalInfoMask.MDH_PATREFANDIMASCAN) ),
            ...
            %%% CATEGORY #2C: just an ordinary ACS line
              ...
              % MKM - add offline DPG scans here 
              if ( ~read_meas__extract_bit(mdh(idx).aulEvalInfoMask(1), EvalInfoMask.MDH_ONLINE) )
                  
                  if ( ~FLAG__patrefscan_dpg ),
                      disp_optional(' PATREFSCAN_DPG detected.');
                      FLAG__patrefscan_dpg = 1;
                  end;
                  patrefscan_dpg(...
                        :, pos(02), ...
                        pos(03), pos(04), pos(05), pos(06), pos(07), pos(08), pos(09), ...
        ...
        elseif ( DO__SMSREFSCAN && ~FLAG__smsrefscan_completed ),
          ...
          if ( read_meas__extract_bit(mdh(idx).aulEvalInfoMask(1), EvalInfoMask.MDH_PHASCOR) ),
            ...
            if ( ~FLAG__data_phascor1d)%  && (FLAG__patrefscan_dpg || ~FLAG__patrefscan) ),
                % MKM - add dpg FLAG in to make sure we are past this as a
                % work around for DPG ref data setting data_begin too early
            ...
            %%%/
            %%%/          % for some multi-shot scans (e.g., ge_functionals), get navigator
          ...
          elseif ( ~read_meas__extract_bit(mdh(idx).aulEvalInfoMask(1), EvalInfoMask.MDH_ONLINE) ),
            ...
            if ( ~FLAG__data ),
              FLAG__data = 1;
              meas_pos; %MKM - display for testing 
            end;
...
%      meas.scan_patrefscan = scan_patrefscan;
    ...
    if ( FLAG__patrefscan_dpg ),
      meas.patrefscan_dpg = patrefscan_dpg;
%      meas.scan_patrefscan = scan_patrefscan;
    end;
