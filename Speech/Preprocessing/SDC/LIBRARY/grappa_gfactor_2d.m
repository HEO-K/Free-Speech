function [ img_grappa, mask, mask_acs, image_weights, g_fnl, weights_p ] = grappa_gfactor_2d2( kspace_sampled, kspace_acs, Rx, Ry, acs_size, kernel_size, lambda_tik, subs )
...
for c = 1:num_chan
    ...
    for ry = 1:Ry-1
        ...
        image_weights(1+end/2 - kernel_hsize(1)*Rx :Rx: 1+end/2 + kernel_hsize(1)*Rx, ry + 1+end/2 - kernel_hsize(2)*Ry :Ry: ry + 1+end/2 + kernel_hsize(2)*Ry, :, c) = W;
    ...
    for rx = 1:Rx-1
        for ry = 0:Ry-1
            ...
            image_weights(rx + 1+end/2 - kernel_hsize(1)*Rx :Rx: rx + 1+end/2 + kernel_hsize(1)*Rx, ry + 1+end/2 - kernel_hsize(2)*Ry :Ry: ry + 1+end/2 + kernel_hsize(2)*Ry, :, c) = W;
