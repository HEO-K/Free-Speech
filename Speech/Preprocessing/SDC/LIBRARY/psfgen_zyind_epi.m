function [nares] = psfgen_zyind_epi(x, param, findxY, Yloc, findxZ, Zloc, receive_pad, Wave_R, cey_pos, zi_pos)
...
%     for cey = floor(y_skip / 2)        
    for ce = 1:length(cey_pos)   
        ...
        [res, ~] = lsqr(@apply_wave_epi, rhs(:), lsqr_tol, lsqr_iter, [], [], [], param);
        
        Ax = apply_wave_epi(res(:), param, 'ntransp');
