function [ img_grappa, mask, mask_acs ] = grappa_2d_vc( kspace_sampled, kspace_acs, Rx, Ry, acs_size, kernel_size, lambda_tik )
...
Rhs = zeros([num_ind, num_chan * 2, Rx*Ry-1]);
...
for ky = ky_begin : ky_end
    for kx = kx_begin : kx_end
        ...
        acs = cat(3, acs, kspace_acs_crop(shiftX_amount + kx-kernel_hsize(1)*Rx:Rx: shiftX_amount + kx+kernel_hsize(1)*Rx, shiftY_amount + ky-kernel_hsize(2)*Ry:Ry: shiftY_amount + ky+kernel_hsize(2)*Ry, 1+num_chan:end));

        Acs(ind,:) = acs(:);

        idx = 1;
        for ry = 1:Ry-1
            Rhs(ind,1:num_chan,idx) = kspace_acs_crop(kx, ky-ry, 1:num_chan);
            
            Rhs(ind,1+num_chan:end,idx) = kspace_acs_crop(kx + shiftX_amount, ky-ry + shiftY_amount, 1+num_chan:end);
        ...
        for rx = 1:Rx-1
            for ry = 0:Ry-1
                Rhs(ind,1:num_chan,idx) = kspace_acs_crop(kx-rx, ky-ry, 1:num_chan);
                
                Rhs(ind,1+num_chan:end,idx) = kspace_acs_crop(kx-rx + shiftX_amount, ky-ry + shiftY_amount, 1+num_chan:end);
...
weights = zeros([prod(kernel_size) * num_chan * 2, num_chan * 2, Rx*Ry-1]);
