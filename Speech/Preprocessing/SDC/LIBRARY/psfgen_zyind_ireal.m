function [nares] = psfgen_zyind_ireal(x, ncol, findxY, Yloc, findxZ, Zloc, ...
    num_chan, shift_amount, receive_pad, Wave_R, param, lsqr_tol, lsqr_iter, y_skip, z_skip, Ry, Rz)
...
for zi = [1 floor(z_skip/2) z_skip]        
    ...
    for cey = floor(y_skip / 2)        
        ...
        [res, ~] = lsqr(@apply_wave_ifft, rhs(:), lsqr_tol, lsqr_iter, [], [], [], param);
        
        Ax = apply_wave_ifft(res(:), param, 'ntransp');
