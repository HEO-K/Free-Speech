...
mkdir(ses_folder);
afile = fullfile(ses_folder, strcat('sub-',subName,'0',sexName,'_ses-1_task-TRUTHLIE_audio.wav'));
sfile = fullfile(ses_folder, strcat('sub-',subName,'0',sexName,'_ses-1_task-TRUTHLIE_timestamp.txt'));
time = strings(4,1);
...
timebarWidth = 80; % [px]
timebarHeight = 10;
timebarRect = [-timebarWidth -timebarHeight timebarWidth timebarHeight];
timebarWin = [rect(1) rect(2) rect(3) rect(4)/4];
timebarRect = CenterRect(timebarRect, timebarWin);
...
%% Instruction
% inst img
img_file = imread(fullfile(pwd, 'Inst','TRUTHLIE','inst.jpg'));
...
%buffer before showing statement.jpg
t0 = GetSecs();
while (GetSecs - t0) < 10
...
% statements img
img_file = imread(fullfile(pwd, 'Inst','TRUTHLIE','statements.jpg'));
img_texture = Screen('MakeTexture', win, img_file);
Screen('DrawTexture', win, img_texture, [], scaled_screen);
Screen('Flip', win);
...
while (GetSecs - t1) < speech_time
    ...
    timebarRectWhite = [timebarRect(1) timebarRect(2) (timebarRect(1)+(timebarWidth*2/speech_time)*(GetSecs-t1)) timebarRect(4)];
    Screen('FillRect', win, black, timebarRect);%black background
    Screen('FillRect', win, white, timebarRectWhite);%white time spent
    Screen('Flip', win);
    if keyCode(Abort)
        break;
    end
